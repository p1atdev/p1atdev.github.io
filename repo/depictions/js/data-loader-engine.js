    ...
    $.each(contentBlocks, function (key, contentInfo) {
        ...
        switch (contentInfo.type) {
            ...
            case "link":
                ...
                url = url + "?" + params.join("&")
                $(key).append(
                    $("<a></a>").attr("href", url).text(contentInfo.text)
                )
                break
            case "list":
            ...
            case "articles":
                ...
                $.each(articles, function (index, article) {
                    var articleTitle = $(article)
                        .find(contentInfo.titleSource)
                        .text()
                    $(key).append(
                        $(contentInfo.titleElement).html(articleTitle)
                    )
                    var container = $(contentInfo.paragraphContainer).attr(
                        "id",
                        ++titleID
                    )
